-- HeXHub Premium - Script Completo y Corregido
...
local function createMainUI()
    ...
    createPartButton.Text = "Crear Part"
...
local function createStealPart()
    if part then
        part:Destroy()
    end
...
-- Función para animar la Part (efecto de baile)
...
local function startSteal()
    ...
    if humanoidRootPart then
        ...
        playerTween = TweenService:Create(humanoidRootPart, tweenInfo, {CFrame = CFrame.new(part.Position + Vector3.new(0, 3, 0))})
        playerTween:Play()
    end
...
-- Función para crear una Part manualmente
local function createPartManual()
    if part then
        part:Destroy()
    end
    part = createStealPart()
...
player.CharacterAdded:Connect(function()
    ...
    if not mainUI then
        ...
        mainUI.CreatePartButton.MouseButton1Click:Connect(createPartManual)
...
print("HeXHub Premium Cargado Correctamente - Todos los botones visibles")